...
authorship_correct_indices = json.loads(open("evaluacion_modelos/gemma3/autoria.json").read())["indices_correctas"]
year_correct_indices = json.loads(open("evaluacion_modelos/gemma3/fecha.json").read())["indices_correctas"]
nationality_correct_indices = json.loads(open("evaluacion_modelos/gemma3/nacionalidad.json").read())["indices_correctas"]
publisher_correct_indices = json.loads(open("evaluacion_modelos/gemma3/editoriales.json").read())["indices_correctas"]
category_correct_indices = json.loads(open("evaluacion_modelos/gemma3/categoria.json").read())["indices_correctas"]
...
df_accuracies = pd.concat([df_accuracies, pd.DataFrame([{"Tipo de pregunta": "P4", "porcentaje correctas": len(category_correct_indices) / 100}])])
df_accuracies = pd.concat([df_accuracies, pd.DataFrame([{"Tipo de pregunta": "P5", "porcentaje correctas": len(publisher_correct_indices) / 100}])])
df_accuracies.to_csv("evaluacion_modelos/gemma3/porcentaje_aciertos.csv", index=False)
...
df_evaluation.to_csv("evaluacion_modelos/gemma3/evaluacion.csv", index=False)
